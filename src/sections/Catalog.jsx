...
export default function Catalog() {
    ...
    return (
        ...
        <section id='Catalog' className="w-full bg-cream-alt py-3 px-3">
            <p className="font-body text-sm font-bold text-center text-gold-main py-2 lg:text-xl">CATÁLOGO</p>
            
            <p className="text-lg text-center text-text-primary font-display py-2 lg:text-5xl">
                <span className="text-text-primary">Nuestras </span>
                <em className="bg-gold-gradient bg-clip-text text-transparent">velas</em>
            </p>
            <p className="font-body text-sm text-center text-text-muted pb-5 lg:text-xl">Cada vela es hecha a mano. Agrega las que quieras en un solo pedido por WhatsApp.</p>

            <div ref={emblaRef} className='overflow-hidden'>
            <div className='flex gap-4 pb-5'>
                {velas.map((vela) => (
                    <div className='flex-none w-full md:w-1/2 lg:w-sm' key={vela.nombre}>
                    <ProductCard {...vela} />
                    </div> 
                ))}
            </div>
